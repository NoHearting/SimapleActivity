...
MainActivity::MainActivity()
{
    ...
    u_id_ = 0;
    name_ = "默认活动";
    create_time_ = QDate::currentDate();
...
MainActivity::MainActivity(int a_id,QString name, QDate dead_time, QString abstract, QString description, QString address):
    name_(name),dead_time_(dead_time),abstract_(abstract),description_(description),address_(address),a_id_(a_id)
{

}

MainActivity::MainActivity(int a_id, int u_id, QString name, QDate create_time, QDate dead_time, int participation, QString abstract, QString description, QString pic_path, int status, QString notice, bool have_child, int total_score):
